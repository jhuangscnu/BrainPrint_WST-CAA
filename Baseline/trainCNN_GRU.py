...
def train(filename,seed,dataset):
    ...
    if dataset == 'DEAP':
        ...
        model_ft = CNN_GRU.CNN_GRU(32)
    elif dataset == 'FACED':
        dataloaders = Load_DataFACED.loadData(batch_size)
        model_ft = CNN_GRU.CNN_GRU(30)
...
if __name__ == "__main__":
    ...
    main_logger.info("CNN_GRU Result")
    DEAP_loss,DEAP_ACC = train("CNN_GRUDEAP", 345, 'DEAP')
    main_logger.info('DEAP:loss:' + str(DEAP_loss) + 'ACC:' + str(DEAP_ACC))
    FACED_loss,FACED_ACC = train("CNN_GRUFACED", 8, 'FACED')
    main_logger.info('FACED:loss:'+str(FACED_loss)+'ACC:'+str(FACED_ACC))
