...
def train(filename,seed,dataset):
    ...
    if dataset == 'DEAP':
        ...
        model_ft = CADCNN.CADCNN(32)
    elif dataset == 'FACED':
        dataloaders = Load_DataFACED.loadData(batch_size)
        model_ft = CADCNN.CADCNN(30)
...
if __name__ == "__main__":
    ...
    main_logger.info("CADCNN Result")

    DEAP_loss,DEAP_ACC = train("CADCNNDEAP", 345, 'DEAP')
    main_logger.info('DEAP:loss:' + str(DEAP_loss) + 'ACC:' + str(DEAP_ACC))

    FACED_loss,FACED_ACC = train("CADCNNFACED", 8, 'FACED')
